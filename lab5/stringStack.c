 ...
 * TA: Majd Alber
 * Description: Implements the functions push, pop, and isEmpty
 */
...
char *  pop()
{
	...
	if (top == 0){
		fprintf(stderr, "Error! Unable to pop on empty stack\n");
		return (char *) 0;
	}
...
{
	if (top >= 100){
		fprintf(stderr, "Error! Unable to push on a full stack\n");
		return;
	}
